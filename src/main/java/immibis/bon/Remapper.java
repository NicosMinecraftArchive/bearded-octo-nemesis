...
public class Remapper {
    ...
    private static String resolveField(Map<String, ClassReferenceData> refClasses, String owner, String name,
                                       String desc, Mapping m) {
    ...
    private static String[] resolveMethod(Map<String, ClassReferenceData> refClasses, String owner, String name,
                                          String desc, Mapping m) {
    ...
    public static ClassCollection remap(ClassCollection cc, Mapping m, Collection<ReferenceDataCollection> refs,
                                        IProgressListener progress) {

        if (!cc.getNameSet().equals(m.fromNS))
            throw new IllegalArgumentException("Input classes use nameset " + cc.getNameSet() + ", but mapping is "
                    + "from " + m.fromNS + "; cannot apply mapping");

        for (ReferenceDataCollection ref : refs)
            if (!ref.getNameSet().equals(m.fromNS))
                throw new IllegalArgumentException("Reference ClassCollection uses nameset " + ref.getNameSet() + " "
                        + "but input uses " + m.fromNS);
	...
	/*public static ClassCollection remap(ClassCollection classes, NameSet toNS, Collection<ClassCollection> refs,
	IProgressListener progress) throws MappingUnavailableException, IOException {
