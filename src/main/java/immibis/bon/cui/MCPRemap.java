...
public class MCPRemap extends CUIBase {
    ...
    protected void run() throws Exception {
        ...
        for (RefOption ro : refOptsParsed) {
            ...
            if (!refNS.equals(inputNS)) {
                ...
                refCC = Remapper.remap(refCC, MappingFactory.getMapping((MinecraftNameSet) refCC.getNameSet(),
                        inputNS, null), Collections.emptyList(), null);
        ...
        ClassCollection outputCC = Remapper.remap(inputCC,
                MappingFactory.getMapping((MinecraftNameSet) inputCC.getNameSet(), outputNS, null), refs, null);
        remapTime += timer.flip();
    ...
    protected void showUsage() {
        ...
        System.out.println("       If <names> is different from <source names>, the file will be remapped "
                + "automatically,");
        ...
        System.out.println("  -mcp . -from OBF -to MCP -side UNIVERSAL -in RedPowerDigital.zip -out "
                + "RedPowerDigital-deobf.zip -ref RedPowerCore.zip -refn MCP:bin/minecraft");
        System.out.println("       Deobfuscates RedPowerDigital.zip, saving the result in RedPowerDigital-deobf.zip.");
        System.out.println("       The current directory contains an MCP installation. RedPowerCore.zip (which is "
                + "obfuscated)");
        System.out.println("       and bin/minecraft (which is not) will also be loaded.");
        System.out.println();
        System.out.println("  -mcp . -from MCP -to OBF -side UNIVERSAL -in AwesomeMod.jar -out AwesomeMod-obf.jar "
                + "-ref bin/minecraft");
