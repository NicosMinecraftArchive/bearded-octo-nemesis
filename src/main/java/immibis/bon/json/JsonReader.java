...
/**
 * Valid JSON values are Maps (from Strings to JSON values), Lists (of JSON values), Booleans, Strings, Doubles and
 * null.
 */
public final class JsonReader {
    ...
    private static String readString(PushbackReader in) throws IOException {
        ...
        while (true) {
            ...
            if (ch == '\\') {
                ...
                case 'u':
                    int value =
                            (fromHexChar(read(in)) << 12) | (fromHexChar(read(in)) << 8) | (fromHexChar(read(in)) << 4) | fromHexChar(read(in));
    ...
    private static Object readJSONInternal(PushbackReader in) throws IOException {
        ...
        if (firstChar == 't') {
            if (read(in) != 'r' || read(in) != 'u' || read(in) != 'e')
                throw new IOException("expected 'rue' after 't'");
            return Boolean.TRUE;

        } else if (firstChar == 'f') {
            if (read(in) != 'a' || read(in) != 'l' || read(in) != 's' || read(in) != 'e')
                throw new IOException("expected 'alse' after 'f'");
            return Boolean.FALSE;

        } else if (firstChar == 'n') {
            if (read(in) != 'u' || read(in) != 'l' || read(in) != 'l')
